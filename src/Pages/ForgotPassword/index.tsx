...
function ForgotPass() {
  ...
  const validationSchema: Yup.Schema<FormValues> = Yup.object({
    emailForg: Yup.string().email('Invalid email address').required('Email is required'),
  });
...
const { data, error } = await supabase.auth.resetPasswordForEmail(values.emailForg, {
        ...
        >
          <Form className="ForgotpassForm">
            ...
            <div className="Forgotpassgroup">
              ...
              <ErrorMessage name="emailForg" component="div" className="error" />
            </div>
            <div className="Forgotpassgroup">
